...
def process_mpd(path):
    ...
    for filename in tqdm.tqdm(sorted(filenames)):
        if filename.startswith("mpd.slice.") and filename.endswith(".json"):
            ...
            count += 1
            seqfile = open('%s/playlists_seq.csv' % path, 'a', newline ='')
            with seqfile:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 2 and sys.argv[2] == "--quick":
        quick = True
    process_mpd(path)
    save_npz('%s/playlist_track.npz' % path, playlist_track.tocsr(False))
    with open('%s/tracks_info/test.json' % path, 'w') as fp:
