...
class UserUserModel(CompletionModel):
  ...
  # and train playlists to complete playlists
...
class ItemItemModel(CompletionModel):
  ...
  # and songs in the train set to complete playlists
...
class MatrixFactorizationModel(CompletionModel):
  ...
  def __init__(self, data_manager, foldername="data/models/mf", emb_size= 256, retrain=False, name='mf_model'):
    self.foldername = foldername
  ...
  def prepare_item_factors(self, retrain=False):
    if retrain:
      try :
        als_model = implicit.als.AlternatingLeastSquares(factors=self.emb_size, calculate_training_loss=True, iterations=10, regularization=12, use_gpu=True, use_cg=True, use_native=False)
        # hyperparameters were formerly tuned with Optuna study
      ...
      self.item_factors = np.load('%s/item_factors_%d.npy' % (self.foldername, self.emb_size))
...
class EnsembleModel(CompletionModel):
  # A class that linearly combines several other models
